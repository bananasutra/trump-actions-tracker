...
# THE OG SEO WORKAROUND (Syntax-Safe with Doubled Braces for CSS)
st.markdown(f"""
    <head>
    ...
    <style>
        /* 1. SURGICAL SCROLL OFFSET - 70px precision to clear the sticky header */
        [id^="section-"] {{
...
comparison_mode = st.sidebar.toggle("📊 Comparison Mode", key="comp_toggle")
...
st.markdown("""<div style="text-align: left;"><h1 style="font-size: 2.8rem; font-weight: 800; margin-bottom: 0px;">🍌 U.S. Democracy Gone Bananas</h1><p style="opacity:0.8; font-size:1.1rem; margin-top:-5px; margin-bottom:5px;">Administrative velocity and institutional dismantle tracker (2025–2026).</p></div>""", unsafe_allow_html=True)
...
# 8. TIMELINE (4-ROW TOOLTIP + COMPARISON LOCK)
...
if not filtered_df.empty:
    ...
    st.markdown("""<p style='font-size:0.75rem; opacity:0.6; font-style:italic; margin-top:-20px;'>💡 <b>Navigation:</b> Hover for 4-row diagnostic data. Click points for source URL.<br>🔗 <b>Note:</b> Some sites (Guardian, NYT, NBC, AP) block internal opening. Use the <b>Search Data Vault</b> below for direct links.</p>""", unsafe_allow_html=True)

# 9. THEMES & GLOSSARY (RE-LOCKED)
...
with c1:
    ...
    st.write("Opposition friction is concentrated in litigation hubs (CA, NY, WA). These remain the primary constraints on administrative velocity.")
with c2:
...
# 12. VAULT (SYNCHRONIZED SEARCH)
st.markdown("<div id='section-search'></div>", unsafe_allow_html=True)
st.divider()
st.subheader("🔍 Search Data Vault")
st.text_input("Synchronized Search (Updates all filters)", key="vault_search", on_change=sync_vault, value=st.session_state.search_term)
if not filtered_df.empty:
